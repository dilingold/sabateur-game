...
public class GameView implements Observer {
    ...
    public void displayView(int totalPlayers, ArrayList<Player> playerNames) {
        ...
        // This checks that the current player hasn't use undo before.
    ...
    public void displayHand() {
        ...
        for (int i = 0; i < hand.cardCount(); i++) {
            ...
            if (hand.getCards().get(i) == null) {
                ...
                String imageName = "/resources/images/cards/" + pathCard.getName() + "-rotate" + pathCard.getRotation()
                        + ".png";
                Image image = new Image(getClass().getResourceAsStream(imageName));
                btn.setGraphic(new ImageView(image));
            } else {
    ...
    // when a path card is clicked from the current player's hand, it rotates
    // once to the right
    ...
    /*
     * when a card is dropped onto the board it goes through a validation
     * process, if it is a valid move, the next player's turn is called
     * 
     * when a card is dropped on the discard icon it is removed from the
     * player's hand and the next player's turn is called
     */
    public void makeDroppable(ImageView target, String dropLocation) {
        ...
        target.setOnDragDropped(event -> {
            ...
            if (dropLocation == "board") {
                ...
                if (dropListener.drop(stage, event, currentPlayer, draggedCardIndex, target, imageViews, rowIndex,
                        colIndex)) {
    ...
    public void setImage(int row, int col, String imageName) {
        /*
         * ImageView playerImageView = new ImageView();
         * playerImageView.setImage(image); playerImageView.setFitWidth(60);
         * playerImageView.setFitHeight(60); target.setGraphic(playerImageView);
         */
        Image image = new Image(getClass().getResourceAsStream(imageName));
    ...
    private void revertTurn(int turns) {
        //add player to list so can't use power again
        int currentPlayer = GameEngine.getCurrentPlayerIndex();
        playersUsedUndo.add(currentPlayer);
        //undo turn
